...
class HumanPlayer(Player):
    def make_move(self):
        move = input("Seçiminizi yapın (Taş, Kağıt, veya Makas): ").lower()
        while move not in ['taş', 'kağıt', 'makas']:
            move = input("Geçersiz seçim! Lütfen Taş, Kağıt, veya Makas seçin: ").lower()
        return move
...
class Game:
    ...
    def play(self):
        # İnsan oyuncusunun adı ve başlangıç bilgileri
        human_name = input("Lütfen oyuncunuzun adını girin: ")
        self.human_player = HumanPlayer(human_name)

        while True:
            # Oyuncu ve bilgisayarın hamlesi
